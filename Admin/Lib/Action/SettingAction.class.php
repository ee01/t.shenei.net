...
class SettingAction extends Action{
    ...
    public function plugins() {
        ...
        $this->assign('site',$site);
        $this->assign('position','全局设置 -> 插件设置');
        $this->display();
    }
    ...
    public function addside() {
        ...
        if ($sidetype && $side && $side['name']) {
            ...
            if ($data) {
                ...
                if (in_array($side['name'],$systemside)) { //系统默认
                    foreach($sides as $val) { //是否重复
                        if ($val['name']==$side['name']) {
                            $this->assign('url',SITE_URL.'/admin.php?s=/Setting/sidebox/t/'.$sidetype);
                            $this->assign('title','很抱歉，已经有相同类型的系统默认侧栏');
                    ...
                    if ($sidetype=='pubside' && ($side['name']=='userfollower' || $side['name']=='userfollowing')) {
                        $this->assign('url',SITE_URL.'/admin.php?s=/Setting/sidebox/t/'.$sidetype);
                        $this->assign('title','很抱歉，广场侧栏不能添加此类型');
                        $this->assign('position','系统跳转');
                        $this->display('Common/return');
                        exit;
                    } else {
                ...
                $this->assign('url',SITE_URL.'/admin.php?s=/Setting/sidebox/t/'.$sidetype);
                $this->assign('title','侧栏添加成功');
            } else {
                $this->assign('url',SITE_URL.'/admin.php?s=/Setting/sidebox');
                $this->assign('title','很抱歉，操作有误');
            }
        } else {
            $this->assign('url',SITE_URL.'/admin.php?s=/Setting/sidebox');
            $this->assign('title','很抱歉，您还没有填写完信息');
        }
        $this->assign('position','系统跳转');
        $this->display('Common/return');
    }
    ...
    public function delside() {
        ...
        if ($t) {
            ...
            if ($data) {
                ...
                $this->assign('url',SITE_URL.'/admin.php?s=/Setting/sidebox/t/'.$t);
                $this->assign('title','恭喜您，删除侧栏成功');
            } else {
                $this->assign('url',SITE_URL.'/admin.php?s=/Setting/sidebox');
                $this->assign('title','很抱歉，操作有误！');
            }
    ...
    public function webset() {
        ...
        $this->assign('url',SITE_URL.'/admin.php?s=/'.$reurl);
        $this->assign('title','恭喜您，设置成功了!');
        $this->assign('position','系统跳转');
        $this->display('Common/return');
    }
