...
class DatabaseAction extends Action {
    ...
    public function export() {
        ...
        if(!$filename || preg_match("/(\.)(exe|jsp|asp|aspx|cgi|fcgi|pl)(\.|$)/i", $filename)) {
            $this->assign('url',SITE_URL.'/admin.php?s=/Database');
            $this->assign('title','很抱歉，备份的文件名有误');
        ...
        if( !is_array($tables) || empty($tables)) {
            $this->assign('url',SITE_URL.'/admin.php?s=/Database');
            $this->assign('title','很抱歉，您还没有选择要备份的数据表');
            $this->assign('position','系统跳转');
            $this->display('Common/return');
            exit;
        }
        ...
        if(trim($sqldump)) {
            ...
            if(@!fwrite($fp, $sqldump)) {
                @fclose($fp);
                $this->assign('url',SITE_URL.'/admin.php?s=/Database');
                $this->assign('title','很抱歉，'.$sqldump.' 文件不可写入！');
                $this->assign('position','系统跳转');
                $this->display('Common/return');
                exit;
            } else {
                ...
                unset($sqldump, $zip);
                $this->assign('url',SITE_URL.'/admin.php?s=/Database/export&filename='.rawurlencode($filename).'&expottype='.rawurlencode($expottype).'&usezip='.$usezip.'&volume='.$volume.'&sizelimit='.$sizelimit.'&startfrom='.rawurlencode($this->_startrow).'&tableid='.rawurlencode($tableid).'&extendin='.$extendin);
                $this->assign('title','数据库第 '.$volume.' 卷备份成功，页面自动跳转！');
                $this->assign('position','系统跳转');
                $this->display('Common/return');
            }
        } else {
            ...
            $this->assign('url',SITE_URL.'/admin.php?s=/Database');
            $this->assign('title','数据库备份完成'.str_replace(ET_ROOT,'',$filelist));
            $this->assign('position','系统跳转');
            $this->display('Common/return');
        }
    }
		...
		if($method == 'serversql' || $method == 'serverzip') {
            if ($method == 'serverzip' && file_exists($sqlfile)) {
                ...
                if($unzip->Count() == 0 || $unzip->GetError(0) != 0 || !preg_match("/\.sql$/i", $importfile = $unzip->GetName(0))) {
                    $this->assign('url',SITE_URL.'/admin.php?s=/Database/import');
			...
			foreach($sqlquery as $sql) {
				if($sql != '') {
					...
					if(mysql_error() && mysql_errno()!=1062) {
						$this->assign('url',SITE_URL.'/admin.php?s=/Database/import');
                        $this->assign('title','数据导入失败，错误原因：<br/>'.mysql_error());
                        $this->assign('position','系统跳转');
                        $this->display('Common/return');
                        exit;
					}
            ...
            $this->assign('url',SITE_URL.'/admin.php?s=/Database/doimport/sqlfile/'.base64_encode($filenext).'/method/'.$method);
            $this->assign('title','已经成功导入第 '.$dumpvolume.' 卷备份，页面自动跳转！');
            $this->assign('position','系统跳转');
            $this->display('Common/return');
            exit;
		} else {
            $this->assign('url',SITE_URL.'/admin.php?s=/Database/import');
            $this->assign('title','未知的导入文件');
            $this->assign('position','系统跳转');
            $this->display('Common/return');
        }
    }
    ...
    public function dooptimize() {
        ...
        $this->assign('url',SITE_URL.'/admin.php?s=/Database/optimize');
        $this->assign('title','恭喜您，数据库优化成功');
        $this->assign('position','系统跳转');
        $this->display('Common/return');
    }
