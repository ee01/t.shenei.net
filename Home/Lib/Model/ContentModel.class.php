...
class ContentModel extends BaseModel {
    ...
    public function sendmsg($content,$morecontent,$from='网页') {
        ...
        if ($this->my['userlock']==2) {
            $ret=array('ret'=>'您已经被禁言，不能发表广播','insertid'=>0);
        ...
        if (!empty($content)) {
            ...
            if ($this->my['lastcontent']==$content) {
                $ret=array('ret'=>'同样的广播发一次就够啦','insertid'=>0);
                return json_encode($ret);
            }
            //attopicurl replace
            ...
            $ret=array('ret'=>'很抱歉广播发表失败','insertid'=>0);
            return json_encode($ret);
        }
    }
    //转播
    public function retwit($cid,$retwit,$type='网页') {
        if ($this->my['userlock']==2) {
            $ret=array('ret'=>'您已经被禁言，不能发表广播','insertid'=>0,'retid'=>0);
            return json_encode($ret);
            exit;
        }
        if ($retwit) {
            $retwit=daddslashes(getsubstr(trim($retwit),0,140,true));
        ...
        if ($cid) {
            ...
            if($data) {
                ...
                $ret=array('ret'=>'很抱歉广播发表失败','insertid'=>0,'retid'=>0);
                return json_encode($ret);
            }
        } else {
            $ret=array('ret'=>'很抱歉广播发表失败','insertid'=>0,'retid'=>0);
        ...
        if ($contentid) {
            ...
            if ($this->my['isadmin']>0 || $data['user_id']==$this->my['user_id']) {
                if ($data) {
                    ...
                    if (count($pics)==3) {
                        @unlink(ET_ROOT.'/'.str_replace(ET_URL,'',$pics[1][0]));
                        @unlink(ET_ROOT.'/'.str_replace(ET_URL,'',$pics[2][0]));
                    }
                }
                //删除mention
    ...
    public function doreply($content,$sid,$isret,$type='网页') {
        if ($this->my['userlock']==2) {
            $ret=array('ret'=>'你已被禁言，不能评论','insertid'=>0);
            return json_encode($ret);
            exit;
        }
        $cm=D('Comments');
        $uModel=D('Users');
        $content=daddslashes(getsubstr(trim($content),0,140,true));
    ...
    public function uploadpic() {
        ...
        if($upload->upload()) {
            ...
            if ($this->site['wateropen']==1) { //水印
                import("@.ORG.Image");
                Image::water($uploadList[0]['savepath'].date('Ymd').'/'.$uploadList[0]['savename'],9,'',SITE_URL.'/'.$this->my['user_name']);
            }
    ...
    function loadonereply($data,$wide=0) {
        ...
        if ($wide==0) {
            ...
            return '<li class="lire">
                <div class="images"><a href="'.SITE_URL.'/'.$data['user_name'].'"><img src="'.sethead($data['user_head']).'" width="30px" height="30px"></a></div>
                <div class="info">
    ...
    function ubb($text) {
        ...
        $rand=randStr(6);
        $r=array(
            "<a class='atlink' href='".SITE_URL."/$1'>$2</a>",
            "ubbpic('\\1','\\2','in')",
    ...
    function wapubb($text) {
        ...
        $r=array(
            ...
            "分享了音乐",
            "ubburl('\\1','\\2','".$this->site['shorturl']."')",
            "<a href='".SITE_URL."/Wap/topic/k/$1'>#$1#</a>",
        );
        $text=preg_replace($p,$r,$text);
        $text=emotionrp($text);
        return $text;
